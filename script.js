...
// ======================================================
// V26.3.3a — SITE SHARE BUTTON
// Initialize immediately with the primary navigation so the Share control
// is not dependent on any later page-specific JavaScript.
// ======================================================
if (nav && !nav.querySelector('.site-share-button')) {
  ...
  const setTemporaryShareLabel = (label) => {
    const labelNode = shareButton.querySelector('.share-label');
    if (!labelNode) return;
    labelNode.textContent = label;
    window.setTimeout(() => {
      labelNode.textContent = 'Share';
    }, 1800);
  };

  const copyPflugelShareLink = async () => {
    try {
      await navigator.clipboard.writeText(shareData.url);
      setTemporaryShareLabel('Link copied');
    } catch (error) {
